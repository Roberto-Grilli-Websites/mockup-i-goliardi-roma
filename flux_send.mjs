...
async function send(name, size, prompt) {
  const resp = await fetch(BASE, {
    ...
    body: JSON.stringify({ prompt, image_size: size, num_inference_steps: 28, guidance_scale: 3.5, num_images: 1, safety_tolerance: "2" })
  })
  const data = await resp.json()
  const rid = data.request_id || `ERROR: ${JSON.stringify(data).slice(0,200)}`
  console.log(`${name}=${rid}`)
  return [name, rid]
}

const pairs = await Promise.all(images.map(([n,s,p]) => send(n,s,p)))
console.log("\n--- ALL REQUEST IDS ---")
